...
df$quality.bucketed <- cut(df$quality, c(2, 5, 7, 10))
```
...
ggplot(aes(x = 0.05 * round(sulphates/0.05), y = chlorides), data = df) +
  geom_line(aes(color = df$quality.bucketed), stat='summary', fun.y = median) +
  xlab("sulphates")
```
...
In this plot we draw the quality of wine v.s. the their `chlorides` level. We see that apart from the lowest and highest quality level where we do not have much data, the wine quality has negative relationship to `chlorides` level. Note that the `x` scale of the plot is in log10 scale because there are the distribution of `chlorides` attribute is very long-tailed.
...
In this plot we draw the relationship between two chemical properties of the wine: `chlorides` v.s. `sulphates`, conditioned on wine quality group. We see that given a quality group, the `chlorides` is mostly independent of `sulphates`, and low-quality wines tend to have higher `chlorides` than high-quality ones, although their `sulphates` have similar span.
